...
import (
	...
	"errors"
	"fmt"
	model "github.com/dreamsofcode-io/orders-api/model"
...
func (r *RedisRepos) GetByID(ctx context.Context, id uint64) (model.Order, error) {
	key := orderIDKey(id)
...
func (r *RedisRepos) DeleteByID(ctx context.Context, id uint64) error {
	key := orderIDKey(id)
...
func (r *RedisRepos) Update(ctx context.Context, order model.Order) error {
	data, err := json.Marshal(order)
	if err != nil {
		return fmt.Errorf("failed to encode order: %w", err)
	}
...
func (r *RedisRepos) GetAll(ctx context.Context, page GetAllPage) (FindResult, error) {
